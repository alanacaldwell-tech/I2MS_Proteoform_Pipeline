...
{
    /// <summary>
    /// Parses the reference CSV file.
    /// Expected format:
    ///   Row 0  : header  (skipped)
    ///   Col 0  : centroid mass (double)
    ///   Col 1  : tolerance in Da (double)
    /// Additional columns beyond col 1 in the header are preserved as-is so
    /// that pre-existing data is not lost when the output is written.
    /// </summary>
